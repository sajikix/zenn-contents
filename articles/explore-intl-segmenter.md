---
title: "Intl.Segmenterはどうやってsegmentしているのか"
emoji: "🕵️‍♂️"
...
初めて Intl.Segmenter の機能を知った人の中には単語単位で分割ができることを知って驚かれる方もいたのではないでしょうか？

本来、日本語や中国語のような単語間にスペースがない言語の単語分割は容易でなく、何らかの辞書や学習データを持つ形態素解析器のようなものが必要になります。ということは各 JavaScript エンジンにそのような機能が実装されているのでしょうか？
...
ICU4XWordSegmenter のヘッダーファイルは [source/intl/icu_capi/c/include/ICU4XWordSegmenter.h](https://searchfox.org/mozilla-central/source/intl/icu_capi/c/include/ICU4XWordSegmenter.h)で定義されています。このファイルのように `source/intl/icu_capi/c` 配下に定義されているヘッダーファイルは ICU4X のコードを FFI 経由で C や C++から呼び出すためのヘッダーファイルです。

https://searchfox.org/mozilla-central/source/intl/icu_capi/c/README.md

よって SpiderMonkey では ICU4X の Segmenter を利用していることがわかりました。（ICU4X については後述します。）
...
ICU は International Components for Unicode の略で Unicode 自体の取り扱いや国際化・地域化を行うオープンソースのライブラリです。現在 C/C++　向けの ICU4C と Java 向けの ICU4J 、Rust で書かれより広い環境での実行を目指す ICU4X などが開発されています。
...
一方 SpiderMonkey は ICU4C ではなく、ICU4X を利用しています。ICU4X は Rust で書かれ、FFI などを通じて Rust 以外の様々な言語で利用されるよう設計された比較的新しい ICU の派生ライブラリです。
...
ここにある通り、辞書にある単語データとそれぞれの出現頻度から動的計画法を利用して単語の分割を行なっています。具体的な実装は以下の部分です。
