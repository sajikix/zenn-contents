...
また、「０」の扱いについても英語のように複数形利用する言語もあれば、フランス語のように「0」の場合単数形を使う言語もあります。
...
このように「文法がどう変わるか」という観点で数を分類すると、言語によって全く異なることがわかります。

### 序数

上記の話は量を表す「基数」についての話でしたが、言語によっては順序を示す「序数」が基数とは異なる体系を持つことがあります。(特にインドヨーロッパ語族に多いようです。)

みなさんご存知の通り英語にも序数(1st,2nd,...)がありますが、その規則はそれなりに複雑です。(特に大きな桁だと筆者も間違えそうになることがあります。)

1. 下 2 桁が 11, 12, 13 の場合と下 1 桁が 0,4~9 の場合は th
2. １以外の場合は下 1 桁が 1 の場合 st, 2 の場合 nd, 3 の場合 rd

そしてこのような序数のルールも言語によって異なります。また逆に日本語のように単独の序数詞を持たず、「番目」といった接尾辞で補てしまう言語もあります。

### 複数形の処理

このように基数や序数で文法が変わる言語においては、国際化の際には複数形の処理が重要になります。例えば、以下のような「英語の単数系のことを全く考えてないコード」を書いてしまうと、特定の値で文法的に正しくない文言が出力されてしまいます。
...
前置きが長くなりましたが、`Intl.PluralRules` はこのような言語ごとの基数と序数のグルーピングをサポートするための API です。
...
生成した Intl.PluralRules `select()` のようなメソッドが生えており(詳しくは記述)、このメソッドに数値を渡すことその数に応じた複数形の規則を示すキーワードを返します。
...
返り値は `zero`, `one`, `two`, `few`, `many`, `other` のいずれかです。ユーザーはこのキーワードを使って、文言の出しわけや変更などを行えます。これらの分類に関して具体例や使い分けに関しては CLDR の Plural Rules を参照してください。

https://cldr.unicode.org/index/cldr-spec/plural-rules#determining-plural-categories

また各ロケールでどのような値が返ってくるか(=基数と序数のルールがどうなっているか)に関しては CLDR の Language Plural Rules を参照してください。
